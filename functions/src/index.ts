...
export const paymentReminders = onSchedule({
    ...
    for (const doc of studentsSnap.docs) {
        ...
        if (student.deactivationDate && new Date(student.deactivationDate) < new Date(year, monthIndex, 1)) continue;
        ...
        if (!expectedFee || expectedFee <= 0) continue; // exento este mes
...
async function sendPaymentReminderEmail(email: string, name: string, pending: number, monthIndex: number) {
    const mailOptions = {
        from: '"Xen Dance Space" <info@xendance.space>',
        to: email,
